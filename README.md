...
```sh
./scripts/build-app.sh                  # dist/TokenBar.app (version defaults to VERSION, currently 0.4.3)
...
- Daily trend buckets by local calendar day (`yyyy-MM-dd`), ascending (aggregate-level detail; the dashboard chart itself is adaptive per range, see above).
...
- **Compact (initial, no scroll):** hero token total for the active source/range, estimated cost (estimate only), `Source` chips (All / Codex / OpenCode / Claude with per-source tokens in range) and `Range` chips (Today / 24H / 7D / 30D / Best / All), a compact visual summary (paired input/output bars with exact counts plus cached/reasoning labelled as subsets, stacked source bar, adaptive mini trend plus a previous-period comparison line), and a clear `Details` expand action plus a one-line updated/notices footer. The trend covers the selected range at the matching grain (Today and 24H by hour including empty hours, 7D/30D/Best by day including empty days, All by month), titled TODAY BY HOUR / LAST 24H BY HOUR / LAST 7D BY DAY / LAST 30D BY DAY / BEST MONTH BY DAY / ALL TIME BY MONTH. Chronological ranges add a compact comparison (Today vs yesterday, 24H/7D/30D vs the preceding window, same source filter); an empty baseline reads "No prior-period data", never 0 percent. The comparison bars use a labeled log scale with a 6% minimum visible width so a much smaller side (for example output dwarfed by input) stays discoverable; bar widths are not shares, counts stay exact, and shares are rounded (one decimal under 1%, so a small nonzero share never reads "0%"). The dashboard opens on the rolling last 7 days (`7D`); every range chip stays available and token/source math is unchanged.
- **Expanded (Details, scrollable):** toggles back to compact via `Show less` in the header. Exposes input/output/cached/reasoning cards (cached reads "subset of input", reasoning "subset of output"), a composition card whose ring splits the total into input vs output only with subset percentages in text, an always-visible source breakdown (zero sources stay listed as `no records`) with a stacked distribution bar, top-5 model bars with share tooltips, the full adaptive trend with date range, grain caption, and comparison line, and sanitized notices.
- Charts are custom SwiftUI (`DashboardCharts.swift`: compact input/output comparison bars, expanded-only composition ring, stacked bar, model bars, adaptive trend bars plus comparison line) fed by `DashboardInsights` shares and `TrendModel` buckets in `TokenBarCore`. Cached and reasoning tokens never render as extra slices or bars; they read as subsets of input and output.
