...
- Charts are custom SwiftUI (`DashboardCharts.swift`: compact input/output comparison bars, expanded-only composition ring, stacked bar, model bars, daily bars) fed by `DashboardInsights` shares in `TokenBarCore`. Cached and reasoning tokens never render as extra slices or bars; they read as subsets of input and output.
