...
# databases, snapshots, and SSH config are never read. Fails on:
...
personal_hits="$(echo "$tracked" | xargs grep -nEi 'manuotel|emanuel[^ ]* otel|otel[^ ]*emanuel' 2>/dev/null || true)"
...
path_hits="$(echo "$tracked" | xargs grep -nE '/Users/[^ /"'"'"':,]+|/home/[^ /"'"'"':,]+' 2>/dev/null || true)"
if [ -n "$path_hits" ]; then
  while IFS= read -r line; do
    case "$line" in
...
cred_hits="$(echo "$tracked" | xargs grep -nE -- '-----BEGIN (RSA |OPENSSH |EC |DSA |PGP )?PRIVATE KEY|AKIA[0-9A-Z]{16}|ghp_[A-Za-z0-9]{20,}|gho_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|glpat-[A-Za-z0-9_-]{15,}|sk-(live|test)-[A-Za-z0-9]{10,}|xox[bpas]-[A-Za-z0-9-]{10,}|AIza[0-9A-Za-z_-]{20,}' 2>/dev/null || true)"
if [ -n "$cred_hits" ]; then
...
machine_hits="$(echo "$tracked" | xargs grep -nE 'Linux manuotel|manuotel@|/home/manuotel|/Users/manuotel|actions/runs/[0-9]+' 2>/dev/null || true)"
