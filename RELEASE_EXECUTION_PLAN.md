...
- Task 2 full tests: PARTIAL (Linux possible subset PASS, Mac subset blocked).
  ...
  - `swift build` / `swift test`: BLOCKED. `swift: command not found` on this Linux host. Requires Mac with Xcode 15+ / macOS 14 SDK per README. CI macOS job remains source of truth; to be confirmed green on PR.
