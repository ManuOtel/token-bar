...
```sh
./scripts/build-app.sh                                   # version defaults to VERSION (currently 0.4.1)
./scripts/build-app.sh --version 0.4.1 --build 29        # explicit release version + bumped build
...
```sh
./scripts/package-release.sh --format zip            # version defaults to the built app, then VERSION (currently 0.4.1)
...
./scripts/verify-dmg.sh --dmg dist/TokenBar-0.4.1-macos.dmg
```
...
ditto -c -k --sequesterRsrc --keepParent dist/TokenBar.app dist/TokenBar-0.4.1-macos.zip
xcrun notarytool submit dist/TokenBar-0.4.1-macos.zip \
...
./scripts/package-release.sh --version 0.4.1 --format zip
./scripts/package-release.sh --version 0.4.1 --format dmg
```
